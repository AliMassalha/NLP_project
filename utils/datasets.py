...
#       reviews[review[1][0]] = {"positive": review[1][2],
#                                       "negative": review[1][3],
#                                       "score": review[1][4]}
#
#
...
class OnlineSimulationDataSet(Dataset):
    def __init__(self, config):
        ...
        for hotel in range(1, N_HOTELS + 1):
            ...
            self.pos_reviews.append(hotel_csv.iloc[:, 2].to_numpy())
            self.neg_reviews.append(hotel_csv.iloc[:, 3].to_numpy())
        # start = time.time()
        self.review_dict = {}
        for ids, pos_reviews, neg_reviews in zip(self.reviews_id, self.pos_reviews, self.neg_reviews):
            for review_id, pos_review, neg_review in zip(ids, pos_reviews, neg_reviews):
                self.review_dict[review_id] = (pos_review, neg_review)
        self.hotels = np.array(self.hotels)
        self.reviews_id = np.array(self.reviews_id)
        # print(time.time()-start)
        # self.pos_reviews =  np.array(self.pos_reviews)
        # self.neg_reviews =  np.array(self.neg_reviews)
    ...
    class SimulatedUser:
        def __init__(self, user_improve, basic_nature, favorite_topic_method, **args):
            ...
            # self.reviews = {}
            # for h in range(1, N_HOTELS + 1):
            #     hotel_df = pd.read_csv(os.path.join("data/game_reviews", f"{h}.csv"),
            #                            header=None)
            #     for review in hotel_df.iterrows():
            #         self.reviews[review[1][0]] = {"positive": review[1][2],
            #                                       "negative": review[1][3],
            ...
            self.ACTIONS = {0: ("correct - oracle", 0, user_strategies.correct_action),
                            ...
                            6: ("sentiment", basic_nature[5], user_strategies.sentiment(loaded_model,loaded_tfidf)),
                            # 6: ("yes man", basic_nature[5], user_strategies.yes_man),
                            }
    ...
    def play_round(self, bot_message, user, previous_rounds, hotel, review_id, pos_neg_review):
        ...
        information = {"bot_message": bot_message,
                       ...
                       "review_id": review_id,
                       "positive": pos_neg_review[0],
                       "negative": pos_neg_review[1],}
            ...
            while (correct_answers < self.SIMULATION_TH) and not (
                ...
                for round_number in range(1, DATA_ROUNDS_PER_GAME + 1):  # start a new round
                    ...
                    user_action = self.play_round(bot_message + signal_error, user, previous_rounds,
                                                  hotel, review_id, self.review_dict[review_id])  # DM plays
